ref ZenMusicConfig g_ZenMusicConfig;

static ZenMusicConfig GetZenMusicConfig()
{
	if (!g_ZenMusicConfig) GetZenConfigRegister().RegisterConfig(ZenMusicConfig);
	return g_ZenMusicConfig;
}
...
modded class ZenConfigRegister
{
	override void RegisterPreload()
	{
		...
		RegisterType(ZenMusicConfig);
	}
}

class ZenMusicConfig_SyncPayload
{
	bool AllowCarInventory;
	
	void ZenMusicConfig_SyncPayload()
	{
		AllowCarInventory = true;
	}
}

class ZenMusicConfig: ZenConfigBase
{
	// -------------------------
	// CONFIG SETTINGS
	// -------------------------
	...
	{
		return JsonFileLoader<ZenMusicConfig>.LoadFile(path, this, err);
	}

	override bool WriteJson(string path, out string err)
	{
		return JsonFileLoader<ZenMusicConfig>.SaveFile(path, this, err);
	}
	
	override bool BuildSyncPayload(out string payload, out string err)
	{
		ZenMusicConfig_SyncPayload snap = new ZenMusicConfig_SyncPayload();

		snap.AllowCarInventory = AllowCarInventory;

		// Serialize payload only (NOT the whole config)
		return JsonFileLoader<ZenMusicConfig_SyncPayload>.MakeData(snap, payload, err, false);
	}

	override bool ApplySyncPayload(string payload, out string err)
	{
		ZenMusicConfig_SyncPayload snap = new ZenMusicConfig_SyncPayload();
		if (!JsonFileLoader<ZenMusicConfig_SyncPayload>.LoadData(payload, snap, err))
		{
			return false;
		}
		
		AllowCarInventory = snap.AllowCarInventory;
		ZMPrint("[ZenMusic] Allow car inventory received - enabled=" + AllowCarInventory);
	...
	override void SetDefaults()
	{
		...
		RadioAllowed.Insert("Zen_BoomBox");
